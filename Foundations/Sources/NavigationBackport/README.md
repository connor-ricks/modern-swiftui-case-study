...
This package uses the navigation APIs available in older SwiftUI versions (such as `NavigationView`) to recreate the new `NavigationStack` APIs introduced in WWDC22, so that you can start targeting those APIs on older versions of iOS, tvOS and watchOS. 
